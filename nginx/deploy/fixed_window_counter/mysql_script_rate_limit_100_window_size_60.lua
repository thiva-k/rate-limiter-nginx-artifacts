...
local mysql_database = "fixed_window_counter_db"
...
local function check_rate_limit(db, token)
    local query = string.format([[
        CALL fixed_window_counter_db.check_rate_limit(%s, %d, %d, @result);
        ]], ngx.quote_sql_str(token), window_size, rate_limit)
    ...
    res, err = db:query("SELECT @result;")
    if not res then
        return nil, err
    end
